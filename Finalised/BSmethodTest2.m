...
for i=1:steps
    ...
    y(i+1, :) = BS1DAdaptive(f, small_tspan, y(i, :), H, kmax, 1e-6, 1e-4);
end
duration = toc;
y=y';
t = 0:H:tspan(2);
...
% n = 1;
% y_slim = y(1:n:end, :);
% y_slim = transpose(y_slim);
% y=y';
%Plot orbits
writematrix(y,'BS10Mil_'+string(stepsize)+'_'+string(duration)+'.csv');
writematrix(t,'BS10Mil_t_'+string(stepsize)+'_'+string(duration)+'.csv');
